...
def ds_process_audio(ds, audio_file, output_file_handle_dict, split_duration):
    ...
    for current_token_index in range(num_tokens):
...
def main():
    ...
    audio_file_name = os.path.join(audio_directory, f"{video_prefix}.wav")
    ...
    for format in args.format:
        output_filename = os.path.join(output_directory, f"{video_prefix}.{format}")
    ...
    for format, file_handle in output_file_handle_dict.items():
